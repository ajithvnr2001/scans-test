...
class StrategyTests(unittest.TestCase):
    ...
    def test_rejects_crash_after_vertical_move(self) -> None:
        ...
        self.assertGreater(signal.metrics["base_drawdown_pct"], 28)
    ...
    def test_strict_daily_early_entry_before_breakout(self) -> None:
        signal = analyze_five_star_setup(
            five_star_breakout_candles(latest_close=160.0, latest_volume=700.0)
        )

        self.assertEqual(signal.stage, "early_entry")
        self.assertTrue(signal.actionable)
        self.assertGreaterEqual(signal.score, 8)
        self.assertLessEqual(abs(signal.metrics["distance_to_pivot_pct"]), 1.0)
