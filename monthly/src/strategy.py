...
DAMAGE_LOOKBACK = 45
RECENT_LOOKBACK = 5
CONTRACTION_MID_LOOKBACK = 10
CONTRACTION_SHORT_LOOKBACK = 5
VERTICAL_LOOKBACK = 18
NEAR_PIVOT_PCT = 0.05
BREAKOUT_SCAN_BARS = 4
FOLLOW_THROUGH_MAX_BARS = 3
FAST_FOLLOW_THROUGH_BARS = 2
MAX_RECENT_RANGE = 0.22
DAMAGED_DRAWDOWN = 0.38
DAMAGED_RANGE = 0.55
EARLY_MAX_BELOW_PIVOT = 0.04
EARLY_MAX_RECENT_RANGE = 0.16
EARLY_MAX_BASE_DRAWDOWN = 0.32
...
def analyze_five_star_setup(candles: list[Candle]) -> SetupSignal:
    ...
    if len(candles) < MIN_CANDLES:
        return SetupSignal(
            ...
            actionable=False,
            reasons=(f"needs at least {MIN_CANDLES} {TIMEFRAME_NAME} candles",),
    ...
    ma20 = mean(closes[-20:])
    ma50 = mean(closes[-50:])
    vertical_gain = _best_prior_gain(closes[:-RECENT_LOOKBACK], max_window=VERTICAL_LOOKBACK)
    base = candles[-BASE_LOOKBACK:-1]
    base_high = max(candle.high for candle in base)
    base_low = min(candle.low for candle in base)
    base_range = _range_pct(base)
    recent_range = _range_pct(candles[-RECENT_LOOKBACK:-1])
    c45 = _window_range_pct(candles, BASE_LOOKBACK, exclude_latest=True)
    c25 = _window_range_pct(candles, CONTRACTION_MID_LOOKBACK, exclude_latest=True)
    c12 = _window_range_pct(candles, CONTRACTION_SHORT_LOOKBACK, exclude_latest=True)
    base_drawdown = (base_high - base_low) / base_high if base_high else 1.0
    post_peak_drawdown = _max_post_peak_drawdown(candles[-DAMAGE_LOOKBACK:-1])
    damaged_base = (
        base_drawdown > DAMAGED_DRAWDOWN
        or base_range > DAMAGED_RANGE
        or post_peak_drawdown > DAMAGED_DRAWDOWN
    )
    pivot = base_high
    ...
    contracting = c45 > c25 * 1.03 and c25 >= c12 * 0.95 and c12 <= 0.22
    tight = recent_range <= MAX_RECENT_RANGE and latest_close >= ma20 * 0.98
    ...
    if stage == "breakout_today":
        ...
        score += 1
        reasons.append(f"price is within {NEAR_PIVOT_PCT:.0%} of the breakout pivot")
    ...
    if (
        ...
        and days_since_breakout <= FAST_FOLLOW_THROUGH_BARS
        and latest_close >= breakout_pivot * 1.05
    ):
        score += 1
    ...
    if early_entry:
        stage = "early_entry"
        reasons.append("strict monthly early-entry candidate near pivot")
...
def _max_post_peak_drawdown(candles: list[Candle]) -> float:
    if not candles:
        return 1.0
    ...
    for candle in candles[1:]:
        if peak > 0:
            max_drawdown = max(max_drawdown, (peak - candle.low) / peak)
...
def _breakout_stage(candles: list[Candle]) -> tuple[str, int, float]:
    ...
    pivot = max(highs[-BASE_LOOKBACK:-1])

    first_breakout_index = None
    first_breakout_pivot = pivot
    start = max(BASE_LOOKBACK, len(candles) - BREAKOUT_SCAN_BARS)

    for index in range(start, len(candles)):
        rolling_pivot = max(highs[index - BASE_LOOKBACK : index])
    ...
    if first_breakout_index is None:
        if latest_close >= pivot * (1 - NEAR_PIVOT_PCT):
    ...
    if days_since_breakout <= FOLLOW_THROUGH_MAX_BARS and extension <= 1.25:
