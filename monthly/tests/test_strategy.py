...
class StrategyTests(unittest.TestCase):
    ...
    def test_strict_monthly_early_entry_before_breakout(self) -> None:
        signal = analyze_five_star_setup(
            five_star_breakout_candles(latest_close=156.0, latest_volume=700.0)
        )

        self.assertEqual(signal.stage, "early_entry")
        self.assertTrue(signal.actionable)
        self.assertGreaterEqual(signal.score, 8)
        self.assertLessEqual(abs(signal.metrics["distance_to_pivot_pct"]), 4.0)
