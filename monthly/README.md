# NSE Monthly 5-Star Setup Scanner

This is the organized monthly timeframe scanner. It is a sibling of `daily/` and `weekly/`.
...
```text
1. Sort key. Matches are now ordered by a five-level strict-entry
   hierarchy: score -> stage tier (BO/EARLY > NEAR > FT) ->
   asymmetric distance bucket -> volume -> proximity. A clean
   low-volume BO at pivot correctly ranks above a high-volume but
   already-extended BO of the same score. Score still dominates.
...
3. Reason string. The 50-MA reason reads "price is above the 50-month
...
- Downloads 10 years of monthly candles by default: `period=10y`, `interval=1mo`.
...
- Adds `early_entry` for clean monthly candidates before breakout. It requires a strong prior move,
  tight monthly base, volume dry-up, a strong close near the pivot, and no post-peak crash damage.
...
- Base window: 18 months.
- Early entry must be within 4% below pivot.
- Recent monthly range must be 16% or tighter.
- Post-peak drawdown damage is checked over 45 months.

### Match ordering

Within `results.json`, matches are sorted by a five-level
strict-entry hierarchy:

```
1. score                    (higher wins)
2. stage tier               BO/EARLY (0) > NEAR (1) > FT (2)
3. distance bucket          asymmetric:
                              0%   to  +3%   best
                             -3%   to   0%   watch
                             +3%   to  +8%   later
                             extended       last
4. breakout_volume_ratio    (higher wins) -- tiebreaker within a bucket
5. |distance_to_pivot_pct|  within-bucket tiebreaker (closer wins)
```

Distance bucket sits above volume, so a clean setup at or just
...
still breaks the tie. Score is still the primary key, so a
high-score FT will beat a low-score BO.
...
- Price above key moving averages (50-month MA).
